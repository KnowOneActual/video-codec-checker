...
def test_qlab_h264_warning():
    ...
    assert any(
        "scrubbing" in issue.message.lower() or "speed" in issue.message.lower() for issue in issues
    )
...
def test_qlab_prores4444_alpha():
    ...
    issues = checker.check(video_info)
    assert any(
        "alpha" in issue.message.lower() or "transparency" in issue.message.lower()
        for issue in issues
    )
...
def test_propresenter_hap_optimal():
    ...
    assert any(issue.level == CompatibilityLevel.COMPATIBLE for issue in issues)
    assert any(
        "best performance" in issue.message.lower() or "gpu" in issue.message.lower()
        for issue in issues
    )
...
def test_twitter_file_size_ok_premium():
    ...
    assert not any(
        issue.level == CompatibilityLevel.INCOMPATIBLE and "exceeds" in issue.message.lower()
        for issue in issues
    )
